...
test_that("works for simple data (sim_data)", {
  ...
  expect_equal(signif(logit_reg_model$coefficients, digits = 2),
               signif(unname(glm_model$coefficients), digits = 2))
  expect_error(fit_logitreg(rowbind(sim_data$design, rep(1,dim(sim_data$design)[2])),
                            c(sim_data$response,2)))
  expect_error(fit_logitreg(cbind(rep(2,length(sim_data$response)), sim_data$design),
                            sim_data$response))
  expect_error(fit_logitreg(cbind(sim_data$design, rep(NA, length(sim_data$response))),
                            sim_data$response))
})
...
test_that("can deal with NA's", {
  ...
  expect_equal(
    ...
    signif(unname(
      glm(trouble1$y ~ trouble1$x[,-1], family = binomial)$coefficients), 2),
    tolerance = 0.005
  )
})
